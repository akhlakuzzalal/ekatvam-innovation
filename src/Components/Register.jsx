...
const Register = () => {
  ...
  const handleCreate = (data) => {
    ...
    if (password === password2) {
      createAccWithEmailPass(email, password)
        .then((userCredential) => {
          // Signed in
      ...
      reset();
    } else setMessage('Password mismatching');
  };
  if (message) {
    setTimeout(() => {
      setMessage('');
      ...
      {!user?.email ? (
        <div className="flex-1 px-3 max-w-[500px] mx-auto">
          ...
          >
            ...
            {message && (
              <p className="text-red-600 font-bold ml-3">{message}</p>
            )}
            ...
            <input
              ...
              value={`${loading ? 'Please wait...' : 'Register with Email'}`}
            />
          </form>
          {error && (
